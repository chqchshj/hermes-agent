...
class WecomCallbackAdapter(BasePlatformAdapter):
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        WeCom accepts AMR/MP3/WAV/AAC. OGG/Opus files are converted to MP3
        ...
        if ext != ".amr":
            ...
            if converted:
                ...
                logger.warning("[WecomCallback] Failed to convert %s to MP3, falling back to text", ext)
                return await super().send_voice(chat_id, audio_path, caption, reply_to, **kwargs)
    ...
    def _convert_to_amr(input_path: str) -> Optional[str]:
        ...
        if not ffmpeg:
            ...
            return None
        # Create temp AMR file (WeCom requires AMR for voice messages)
    ...
    async def _handle_callback(self, request: web.Request) -> web.Response:
        ...
        logger.info("[WecomCallback] Received callback request from %s", request.remote)
        msg_signature = request.query.get("msg_signature", "")
        timestamp = request.query.get("timestamp", "")
        nonce = request.query.get("nonce", "")
